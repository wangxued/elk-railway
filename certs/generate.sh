#!/bin/bash

echo "Generating CA certificate"
openssl req -x509 -newkey rsa:3072 -days 3650 -nodes -keyout ca.key -out ca.crt -subj "/CN=Elastic-CA" -sha256

echo "Generating Elasticsearch certificate"
openssl req -newkey rsa:3072 -nodes -keyout elasticsearch.key -out elasticsearch.csr -subj "/CN=elasticsearch" -sha256
openssl x509 -req -in elasticsearch.csr -CA ca.crt -CAkey ca.key -CAcreateserial -out elasticsearch.crt -days 3650 -sha256

echo "Generating Kibana certificate"
openssl req -newkey rsa:3072 -nodes -keyout kibana.key -out kibana.csr -subj "/CN=kibana" -sha256
openssl x509 -req -in kibana.csr -CA ca.crt -CAkey ca.key -CAcreateserial -out kibana.crt -days 3650 -sha256

echo "Generating Logstash certificate"
openssl req -newkey rsa:3072 -nodes -keyout logstash.key -out logstash.csr -subj "/CN=logstash" -sha256
openssl x509 -req -in logstash.csr -CA ca.crt -CAkey ca.key -CAcreateserial -out logstash.crt -days 3650 -sha256

echo "Generating Filebeat certificate"
openssl req -newkey rsa:3072 -nodes -keyout filebeat.key -out filebeat.csr -subj "/CN=filebeat" -sha256
openssl x509 -req -in filebeat.csr -CA ca.crt -CAkey ca.key -CAcreateserial -out filebeat.crt -days 3650 -sha256

chmod 644 *.crt *.key

